...
function processLines(state) {
    const {winningCall, winningBoard} = state.seq.reduce((partial, call) => {
        if (partial.winningCall) { return partial };
        const newBoards = partial.boards.map(board => markBoard(board, call));
        const wonBoard = newBoards.filter(boardWon);
        if (wonBoard.length) {
            return {
                winningBoard: wonBoard[0],
                boards: newBoards,
                winningCall: call,
            }
        }
        return { winningBoard: undefined, boards: newBoards, winningCall: undefined};
    }, { 
        winningBoard: undefined, 
        boards: state.boards, 
        winningCall: undefined
    });
    const boardSum = winningBoard.filter(i => !i[1]).map(i => i[0]).reduce((a,b) => a+ b);
    return boardSum * winningCall
}
