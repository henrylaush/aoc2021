...
function processLines({ table, image }) {
  const once = convolution(padImage(image, 0), table, 0);
  const twice = convolution(padImage(once, table[0]), table, table[0]);
  return twice.flatMap(id => id).filter(Boolean).length;
}
