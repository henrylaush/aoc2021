...
async function parseOperator(bStr) {
  ...
  const common = { type: parseInt(type, 2), lengthId: lenTypeId, version: parseInt(version, 2) };
  if (lenTypeId === '0') {
...
function calculate(node) {
  switch (node.type) {
    case 0:
      return node.subPackets.map(calculate).sum();
    case 1:
      return node.subPackets.map(calculate).reduce((a, b) => a * b);
    case 2:
      return Math.min(...node.subPackets.map(calculate));
    case 3:
      return Math.max(...node.subPackets.map(calculate));
    case 4: 
      return node.value;
    case 5:
      return calculate(node.subPackets[0]) > calculate(node.subPackets[1]) ? 1 : 0;
    case 6:
      return calculate(node.subPackets[0]) < calculate(node.subPackets[1]) ? 1 : 0;
    case 7:
      return calculate(node.subPackets[0]) === calculate(node.subPackets[1]) ? 1 : 0;
